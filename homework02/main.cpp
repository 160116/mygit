...
typedef struct
{
    QStringList stu;        //定义字符串列表
} studData;
QDebug operator << (QDebug d, const studData &data)   //运算符<<重载,以直接输出studData结构
{
...
void ScoreSorter::doSort()
{
    ...
    {
        myCmp stducmp(i-1);
        std::sort(this->data.begin() , this->data.end() , stducmp );   //std::sort排序
